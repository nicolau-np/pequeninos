...
<head>
    ...
    <title>DECLARAÇÃO COM NOTAS {{ $getHistorico->ano_lectivo }} - [
        {{ strtoupper($getHistorico->estudante->pessoa->nome) }} ] </title>
    <style>
        @page {
...
<body>
    <div class="default-page">
        ...
        <div class="titulo">
            <p style="text-align: center; font-weight:bold; font-size:18px;">DECLARAÇÃO Nº
                {{ str_pad($getDeclaracao->numero, 4, 0, STR_PAD_LEFT) }}/{{ $getDeclaracao->ano_emissao }}</p>
        </div>
        ...
        <div class="corpo">
            ...
            <div class="primeiro-paragrafo">
                ...
                Filho de @if ($getHistorico->estudante->pessoa->pai)
                {{ $getHistorico->estudante->pessoa->pai }} @else [########################] @endif
                e de @if ($getHistorico->estudante->pessoa->mae)
                {{ $getHistorico->estudante->pessoa->mae }} @else [########################] @endif
                ...
                Identidade Nº
                @if ($getHistorico->estudante->pessoa->bilhete)
                {{ $getHistorico->estudante->pessoa->bilhete }}, @else [########################] @endif Passado pelo
                ...
                lectivo de [{{ $getHistorico->ano_lectivo }}] a {{ $getTurma->classe->classe }}, com a seguinte
                classificação:
            </div>
            <br />
                                ...
                                <tr>
                                    ...
                                    @else
                                        @foreach ($final as $valorf)
                                            @php
                                                $v1_estilo = ControladorNotas::nota_10($valorf->mf);
                                                $v2_estilo = ControladorNotas::notaRec_5($valorf->rec);

                                                $v1_valor = ControladorNotas::estado_nota_qualitativa($valorf->mf);
                                                $v2_valor = ControladorNotas::estado_nota_qualitativaRec($valorf->rec);
                                            @endphp

                                            @if ($valorf->rec == null)
                                                <td class="{{ $v1_estilo }}">{{ $v1_valor }}</td>
                                            @else
                                                <td class="{{ $v2_estilo }}">{{ $v2_valor }}</td>
                                            @endif
                                        @endforeach
                                    @endif
                                </tr>
            ...
            <div class="segundo-paragrafo">
                Por ser verdade e me ter sido solicitado, mandei passar a presente declaração que vai por mim assinada e
                autenticada com o carimbo a óleo, em uso nesta instituição escola.
            </div>
            <br />
            </p>
